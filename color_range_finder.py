...
def on_click(event,x,y,falgs,param):
    if event ==cv2.EVENT_LBUTTONDOWN:
        ...
        for i,j in enumerate('HSV'):
            cv2.setTrackbarPos("%s_%s" % (j, "MIN"), "Trackbars", hsv[y][x][i])
...
def main():
    ...
    if args['image']:
       ...
       '''
    range_filter = 'HSV'
    camera = cv2.VideoCapture(0)    # change to 0
   ...
   # args['webcam'] = true
    while True:
        ...
        #image = cv2.bilateralFilter(image,3,150,150)
        ...
        cv2.setMouseCallback('Original',on_click,image)
