...
def bot(shared_memory,bot_no):
	...
	while True:
		...
		frame = np_array.astype("uint8").copy()  # example of some processing done on the array
...
def main():
	...
	proc1 = mp.Process(target=bot, args=( shared_memory,0))
	proc2 = mp.Process(target=bot, args=( shared_memory,1))
	start = True
	while True:
