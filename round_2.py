...
mp_array = mp.Array("I", int(np.prod((720,1280,3))))   # , lock=mp.Lock()  (480, 640, 3)
...
def monitor(): 			# monitors the bot coordinates and updates the control
	...
	while True:	
		...
		if flag:
			if ((bot_state[0].pos[0] + offset_map[bot_state[0].curr_dir][0]) < (bot_state[1].pos[0] + offset_map[bot_state[1].curr_dir][0]) + 			boundary[bot_state[1].mov_dir] 					    and 
			    (bot_state[0].pos[0] + offset_map[bot_state[0].curr_dir][0]) + 			boundary[bot_state[0].mov_dir] 					    > (bot_state[1].pos[0] + offset_map[bot_state[1].curr_dir][0])  and
				(bot_state[0].pos[1] + offset_map[bot_state[0].curr_dir][1]) < (bot_state[1].pos[1] + offset_map[bot_state[1].curr_dir][1]) +  			boundary[~bot_state[1].mov_dir] 				    and
							 boundary[~bot_state[0].mov_dir]			     + (bot_state[0].pos[1] + offset_map[bot_state[0].curr_dir][1])	> (bot_state[1].pos[1] + offset_map[bot_state[1].curr_dir][1])) :
				'''											based on this logic
...
def main():
	...
	proc3 = mp.Process(target=monitor)
	start = True
	c_time = time.time()
	while True:
