		...
		for i,j in enumerate(color_def):
			...
			for k,c in enumerate(cnts):
				if len(c)<10:
					...
					if cx>some_value &&cy>some_value:			#need to define some_value
						return bot_paths[bot_no][i]
					elif cx<some_value &&cy<some_value:
						return bot_paths[bot_no][i]
...
def main():
	...
	p1 = mp.Process(target=bot,args=(0,))
	p2 = mp.Process(target=bot,args=(1,))
	p1.start()
	p2.start()
	while True:

		got, frame = cap.read()
		image.put(cv2.cvtColor(frame, cv2.COLOR_BGR2HSV))
